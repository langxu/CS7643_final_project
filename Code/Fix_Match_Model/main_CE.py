...
BATCH_SIZE = 64
NUM_EPOCHS = 200
NUM_LABELED = 250  # Increased to 500 per class
MU = 7
...
def train():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        val_loss, val_acc, val_per_class_acc = evaluate(model, validation_loader, device)
        ...
        logger.info(f'Epoch {epoch + 1}: Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.2f}%, '
                    f'Cat Val Acc: {val_per_class_acc[0]:.2f}%, Dog Val Acc: {val_per_class_acc[1]:.2f}%, '
        ...
        if val_acc > best_val_acc:
            ...
            ema_model.resume(model)
    model.load_state_dict(torch.load('best_model_ema.pth'))
    ema_model.assign(model)
    test_loss, test_acc, test_per_class_acc = evaluate(model, test_loader, device)
    logger.info(f'Final Test Loss: {test_loss:.4f}, Test Acc: {test_acc:.2f}%, '
                f'Cat Test Acc: {test_per_class_acc[0]:.2f}%, Dog Test Acc: {test_per_class_acc[1]:.2f}%')
